...
public abstract class Ant extends Observable implements Runnable {
	...
	protected Vector<Object> m_pathVect;
	...
	public static Vector<Object> s_bestPathVect = null;
	...
	public void init() {
		...
		m_pathVect = new Vector<Object>(graph.nodes());
	...
	public void run() {
		...
		synchronized (graph) {
			...
			if (better(m_dPathValue, s_dBestPathValue)) {
				...
				s_outs.println("Ant + " + m_nAntID + "," + s_dBestPathValue
						+ "," + s_nLastBestPathIteration + ","
						+ s_bestPathVect.size() + "," + s_bestPathVect);
			}
		}
